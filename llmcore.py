...
class CopilotSDKSession(BaseSession):
    ...
    def _emit_cli_logs(self, client):
        ...
        try:
            ...
            sys.stderr.write(merged_logs if merged_logs.endswith('\n') else merged_logs + '\n')
            sys.stderr.flush()
        except OSError:
            pass
    ...
    def raw_ask(self, messages):
        ...
        if thread_result["err"] is not None:
            if self._is_session_idle_timeout(thread_result["err"]):
                ...
                partial_text = thread_result["text"] or (''.join(buffered_deltas) if streamed else "")
                if partial_text:
                    yield partial_text
                    return [{"type": "text", "text": partial_text}]
                return []
